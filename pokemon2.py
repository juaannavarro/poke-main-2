...
df_pokemon2 = pd.melt(df_pokemon2, id_vars=["Name", "Type 1", "Type 2"], var_name="Stat")
with sns.color_palette([
    ...
    plt.ylim(0, 275)
    sns.stripplot(x="Stat", y="value", data=df_pokemon2, hue="Type 1", dodge=True)
    ...
    a3=sns.regplot(x="Attack", y="Total", data=df, label="Attack", x_jitter=.2)
    a3=sns.regplot(x="Defense", y="Total", data=df, label="Defense", x_jitter=.2)
    a3=sns.regplot(x="Speed", y="Total", data=df, label="Speed", x_jitter=.2)
    a5=sns.regplot(x="Sp. Atk", y="Total", data=df, label="Sp. Atk", x_jitter=.2)
    a6=sns.regplot(x="Sp. Def", y="Total", data=df, label="Sp. Def", x_jitter=.2)
    ...
    plt.show()
    ax=sns.catplot(x='Generation',data=df,height=8,aspect=1.2,kind='count').set_axis_labels('Generation', '# of Pokemon')
...
sns.heatmap(crosstab, annot=True,cmap=sns.color_palette("flare"), cbar=False, linewidths=.5)
...
plt.show()
x1=attack_total=sns.relplot(data=df, x="Attack", y="Total", hue="Legendary", height=7)
x2=defense_total=sns.relplot(data=df, x="Defense", y="Total", hue="Legendary", height=7)
x3=speed_total=sns.relplot(data=df, x="Speed", y="Total", hue="Legendary", height=7);
plt.show()
...
def comp(Type,Attribute):
    ...
    sns.swarmplot(x=Type,y=Attribute,ax=ax[0],data=df, palette=Pokemon_colors)
    sns.violinplot(x=Type,y=Attribute,ax=ax[1],data=df, palette=Pokemon_colors)
...
df[['Name','Type 1','Type 2','HP','Attack','Defense','Speed','Generation','Total','Legendary']].sort_values(by=['Total'],ascending = False).head(10).reset_index(drop=True).style.background_gradient(cmap=cm)
